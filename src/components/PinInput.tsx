...
const PinInput = React.forwardRef<HTMLDivElement, PinInputProps>(({ className, children, ...props }, ref) => {
    ...
    return (
        <PinInputContext.Provider value={true}>
            <div ref={ref} aria-label="Pin Input" className={className} {...rest} style={style}>
                {clones}
...
const PinInputFieldNoRef = <T extends React.ElementType = "input">(
    { className, component, ...props }: PinInputFieldProps<T> & (React.ComponentType<T> extends undefined ? never : React.ComponentProps<T>),
