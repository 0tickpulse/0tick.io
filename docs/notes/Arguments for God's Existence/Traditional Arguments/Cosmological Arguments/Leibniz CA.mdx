...
An object cannot be its own reason for existence, because that would imply that it is both prior and posterior to itself.
...
All this means is that if we accept the PSR, then it leads to an infinite regress.
Hence, we have to restrict the PSR to only apply to certain types of things, like contingent things.
This is not mentioned in the first premise. Instead, it states that "**anything**" that exists has a sufficient reason for its existence.
...
Defining this collection requires an ontological commitment to the existence of the collection itself.
