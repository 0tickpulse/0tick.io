...
The order of this section is as follows:

1. Show the two definitions of the dot product, assuming they're the same.
2. Use both definitions to prove the properties of the dot product.
3. Use these definitions to prove the equivalence of the two definitions.

It's not circular reasoning becuase the properties can be proven with one definition.
Each property's subsection has an algebraic proof with the first definition, and a geometric proof with the second definition.
...
\abs{\va{v} \cdot \va{w}} &= \norm{\va{v}} \norm{\va{w}} \iff \va{v} = c\va{w}, c \in \mathbb{R} \label{eq:cauchy-schwarz-inequality-2}
\end{align}
$$
...
This is actually really easy to prove geometrically.
