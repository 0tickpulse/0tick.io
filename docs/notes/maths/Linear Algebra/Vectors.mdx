...
Vectors have mulitple definitions:
...
One way to think of the dot product is to "project" one vector onto the other.
...
So, $\va{v} \cdot \va{w}$ is the projection of $\va{v}$ onto $\va{w}$.
