...
<table>
    ...
    {[
        ...
        ["e^x", "e^x + C"],
        ["\\frac{1}{x}", "\\ln|x| + C"]
    ].map(([func, antiderivative]) => (
        <tr>
            <td>
                <MathEquation>{func}</MathEquation>
            </td>
            <td>
                <MathEquation>{antiderivative}</MathEquation>
            </td>
        </tr>
    ))}
</table>
...
- **$u$-substitution** is used to simplify the integrand by substituting a variable $u$:
    $$
    \begin{equation}
    \int f(g(x))g'(x) dx = \int f(u) du
    \end{equation}
    $$
- **Algebraic manipulation**, such as long division or completing the square, can simplify the integrand.
