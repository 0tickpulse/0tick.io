...
While some textbooks suggest that $\dv{y}{x}$ shouldn't be thought of as a fraction, it's often much more intuitive to think of it as one.
...
Furthermore, when $\dd{y}$ and $\dd{x}$ are seen by themselves, they are much easier to understand. This is important especially in implicit differentiation.
...
\therefore \dv{x} \tan{x} &= \frac{\cos{x} \cos{x} - (\sin{x})(-\sin{x})}{\cos^2{x}} \\
