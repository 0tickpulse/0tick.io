...
The dot product "projects" one vector onto another. If you have two vectors $\va{a}$ and $\va{b}$, the dot product is $\va{a} \cdot \va{b} = |\va{a}| |\va{b}| \cos(\theta)$, where $\theta$ is the angle between the two vectors.
...
$$
\begin{equation}
\text{Acceleration} = \frac{d\va{x}}\dd{t} \cdot \va{g} = -g \sin(\theta)
\end{equation}
$$
...
L \frac{d^2\theta}{dt^2} &= -\frac{g}{L} \sin(\theta) - b L \dv{\theta}{t} \\
\frac{d^2\theta}{dt^2} &= -\frac{g}{L} \sin(\theta) - b \dv{\theta}{t}
\end{align}
$$
