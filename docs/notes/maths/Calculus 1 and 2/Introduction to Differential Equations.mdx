...
{(() => {
    ...
    return (
        <div className="card">
            ...
            <div className="card__footer">
                <ControlPanel>
                    <SliderInput latex label="\Delta x" value={dx} min={0.1} max={1} step={0.1} onUpdate={setDx} />
                </ControlPanel>
            </div>
        </div>
    )
...
Euler's method is a very simple method to approximate the solution to a DE.
However, it can be very tedious to do by hand, especially for more complex DEs or more steps.
Thus, it's often done with technology. We'll use Python to demonstrate this, but this can be done with anything.

Consider the DE:

$$
\begin{equation}
\dv{y}{x} = y^2 + x^2, \quad y(0) = 1
\end{equation}
$$

Let's approximate the solution after 300 steps with $\Delta x = 0.01$.
...
- Storing the $x$ and $y$ values in a list.
...
\begin{equation}
\begin{split}
\frac{\posY^2}{2} &= \frac{1}{2} e^{-\posX^2} + C \\
...
Pierre François Verhulst, a Belgian mathematician, proposed a model that describes this type of growth.
        ...
        <div className="card">
            ...
            <div className="card__footer">
                ...
                <div className="input-group">
                    <ControlPanel>
                        <SliderInput latex label="k" value={k} min={0.1} max={10} step={0.1} onChange={setK} />
                    </ControlPanel>
                </div>
            </div>
        </div>
...
Recall that slope fields are used to visualize the behavior of a DE.
In a system of DEs, we can plot the slope field by setting each axis to the population of one species.
This is known as a phase space or phase portrait.
...
{(() => {
    ...
    return (
        <div className="card">
            ...
            <div className="card__footer">
                <ControlPanel>
                    <SliderInput latex label="\alpha" value={alpha} min={0.1} max={2} step={0.1} onChange={setAlpha} />
                    <SliderInput latex label="\beta" value={beta} min={0.1} max={2} step={0.1} onChange={setBeta} />
                    <SliderInput latex label="\delta" value={delta} min={0.1} max={2} step={0.1} onChange={setDelta} />
                    <SliderInput latex label="\gamma" value={gamma} min={0.1} max={2} step={0.1} onChange={setGamma} />
                </ControlPanel>
            </div>
        </div>
    )
