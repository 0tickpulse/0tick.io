...
Recall the definition of the gradient in Cartesian coordinates:

$$
\begin{equation}
\grad{f(x, y, z)} = \mqty[ \pdv{f}{x} \\ \pdv{f}{y} \\ \pdv{f}{z} ] = \pdv{f}{x} \vu{i} + \pdv{f}{y} \vu{j} + \pdv{f}{z} \vu{k}
\end{equation}
$$

One would expect that the gradient in cylindrical coordinates would be similar:

$$
\begin{equation}
\grad{f(r, \theta, z)} \overset{?}{=} \pdv{f}{r} \vu{e}_r + \pdv{f}{\theta} \vu{e}_\theta + \pdv{f}{z} \vu{e}_z
\end{equation}
$$

Unfortunately, like everything else, things are not as simple as they seem.
