...
## Path Independence of Conservative Vector Fields
...
<Boxed>
A vector field $\va{F}$ is conservative if there exists a scalar function $f$ such that $\va{F} = \grad{f}$.
</Boxed>

## All Conservative Fields are Gradient Fields (Optional Section)
...
In this proof, the above is the premise which defines the function $f$.
...
$$
\begin{equation}
\pdv{f}{\va{v}}(\va{b}) = \lim_{h \to 0} \frac{1}{h} g(h) \equiv \lim_{h \to 0} \frac{g(h) - g(0)}{h}
\end{equation}
$$
...
If $\va{F}$ is conservative, by definition, there exists a scalar function $U$ such that $\va{F} = \grad{U}'$.
This function $U'$ is called the **potential function** of the vector $\va{F}$.
...
<details>
<summary>Why the $'$?</summary>

One might hear "potential function" and think of the function $U'$ as the potential energy.
In reality, the actual definition of the potential energy is $U = -U'$.
