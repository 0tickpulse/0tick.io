...
- **Directional Derivative**: The rate of change of a function in a certain direction:
    ...
    $$
    \begin{equation}
    \grad_{\va{u}} f = \grad{f} \cdot \va{u}
    \end{equation}
    $$

    Where $\va{u}$ is a unit vector. Using this we can show that the directional derivative is maximized when $\va{u}$ is in the direction of the gradient:
    ...
    Where $\theta$ is the angle between the gradient and $\va{u}$.
- **Multivariable Chain Rule**: The chain rule for functions of multiple variables. For a function $f(x(t), y(t))$:
...
- **Curvature**: The rate of change of the unit tangent vector with respect to arc length. The curvature of a curve $\va{r}(t)$ is:
    ...
    Where $\va{T}$ is the unit tangent vector.
...
- **Partial Derivatives of Parametric Surfaces**: The partial derivatives of a vector-valued function with respect to its parameters.
    For a function $\va{f}(t, s) = \mqty[ x(t, s) \\ y(t, s) \\ z(t, s) ]$:

    $$
    \begin{equation}
    \pdv{t} \va{f}(t, s) = \mqty[ \pdv{x}{t} \\ \pdv{y}{t} \\ \pdv{z}{t} ]
    \end{equation}
    $$
