...
We can identify $p(x) = 2t$ and $q(x) = t$.
The integrating factor is

$$
\begin{equation}
I(x) = \exp(\int 2t \dd{x}) = \exp(x^2).
\end{equation}
$$
...
$$
\begin{equation}
\exp(x^2) \dv{y}{x} + 2t \exp(x^2) y = x \exp(x^2).
\end{equation}
$$
...
To solve an exact first-order ODE, we can integrate $M(x, y)$ with respect to $t$ to find $f(x, y)$, and then set $f(x, y) = c_0$ to find the solution.
...
$$
\begin{equation}
\dv{y}{x} = -\frac{2t y^2 + 1}{2x^2 y}.
\end{equation}
$$

</summary>

We can identify $M(x, y) = 2t y^2 + 1$ and $N(x, y) = 2x^2 y$, so

$$
\begin{equation}
(2t y^2 + 1) \dd{x} + (2x^2 y) \dd{y} = 0.
\end{equation}
$$
...
To find $f(x, y)$, we just integrate $M(x, y)$ with respect to $t$:

$$
\begin{equation}
f(x, y) = \int (2t y^2 + 1) \dd{x} = x^2 y^2 + x + g(y),
\end{equation}
$$
...
$$
\begin{equation}
f(\lambda t, \lambda y) = \lambda^n f(x, y).
\end{equation}
$$
...
$$
\begin{equation}
f(\lambda t, \lambda y) = (x\lambda)^2 + (y\lambda)^2 = \lambda^2 (x^2 + y^2) = \lambda^2 f(x, y).
\end{equation}
$$
...
\begin{align}
f(\lambda t, \lambda y) &= \lambda^n f(x, y) \\
g(\lambda t, \lambda y) &= \lambda^n g(x, y).
\end{align}
$$
...
$$
\begin{equation}
\frac{f(\lambda t, \lambda y)}{g(\lambda t, \lambda y)} = \frac{\lambda^n f(x, y)}{\lambda^n g(x, y)} = \frac{f(x, y)}{g(x, y)},
\end{equation}
$$
