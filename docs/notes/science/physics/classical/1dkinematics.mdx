...
Usually, position is denoted as a function of time $\bvec{r}(t)$, where $t$ is time.
In one-dimensional kinematics, the position can be written as $x(t)\hat{\bvec{i}}$. Here, $\hat{\bvec{i}}$ is the unit vector in the direction of motion.
...
{(() => {
    ...
    return <Mafs>
        ...
        />
        <LaTeX
            tex={`\\bvec{r}(t) = ${x.point[0].toFixed(2)}\\hat{\\bvec{i}}`}
            at={[x.point[0] / 2, 0.3]}
        />
...
Displacement is the change in position of an object, or $\Delta \bvec{r}$. It is also a vector quantity.
...
$$
\Delta \bvec{r} = x(t_2)\hat{\bvec{i}} - x(t_1)\hat{\bvec{i}} \equiv \Delta x\hat{\bvec{i}}
$$
...
$$
\bvec{v}_{\text{avg}} = \frac{\Delta \bvec{r}}{\Delta t}
$$
...
$$
\bvec{v} = \frac{d\bvec{r}}{dt}
$$

And in one-dimensional kinematics:

$$
\bvec{v} = \frac{d}{dt}(x\hat{\bvec{i}}) = \frac{dx}{dt}\hat{\bvec{i}}
$$
...
$$
\bvec{a}_{\text{avg}} = \frac{\Delta \bvec{v}}{\Delta t}
$$
...
$$
\bvec{a} = \frac{d\bvec{v}}{dt}
$$

And in one-dimensional kinematics:

$$
\bvec{a} = \frac{d}{dt}(v\hat{\bvec{i}}) = \frac{dv}{dt}\hat{\bvec{i}}
$$
...
$$
\bvec{x}(t) = \bvec{x}_0 + \bvec{v}_0 t + \frac{1}{2} \bvec{a} t^2
$$
