...
export function SingleVector() {
    ...
    const magnitudeText = `\\|\\bvec{v}\\| = \\sqrt{(${v.point[0].toFixed(2)})^2 + (${v.point[1].toFixed(2)})^2}`;
    ...
    return (
        <Mafs>
            ...
            />
            <LaTeX
                tex={String.raw`\bvec{v}_x = ${v.point[0].toFixed(2)}`}
                at={[v.point[0] / 2, -0.3]}
            />
            ...
            />
            <LaTeX
                tex={String.raw`\bvec{v}_y = ${v.point[1].toFixed(2)}`}
...
Vectors are typically denoted by a lowercase boldface letter, like $\bvec{v}$.
...
The magnitude of a vector $\bvec{v}$ is denoted by $\|\bvec{v}\|$, and it is the length of the vector.
Sometimes a single bar is used, like $|\bvec{v}|$, but this can be confused with the absolute value of a scalar.
...
Addition is commutative, meaning $\bvec{v} + \bvec{w} = \bvec{w} + \bvec{v}$. This can be visually seen:
...
The addition of vectors is also associative, meaning $(\bvec{v} + \bvec{w}) + \bvec{u} = \bvec{v} + (\bvec{w} + \bvec{u})$.
...
export function VectorAdditionAssociative() {
    ...
    return (
        <Mafs>
            ...
            />
            <LaTeX
                tex={String.raw`\bvec{v} + \bvec{w} + \bvec{u}`}
...
The identity element for vector addition is the zero vector, denoted by $\bvec{0}$.
The zero vector has a magnitude of 0 and is the only vector that has a magnitude of 0.

Formally, $(\exists \bvec{0} \in \mathbb{R}^n) \, \forall \bvec{v} \in \mathbb{R}^n \, \bvec{v} + \bvec{0} = \bvec{v}$.
This means that adding the zero vector to any vector gives the original vector.

## Inverse Element

There's also an inverse element for vector addition, denoted by $-\bvec{v}$.
The inverse element is the vector that, when added to the original vector, gives the zero vector.

Formally, $(\forall \bvec{v} \in \mathbb{R}^n) \, \exists -\bvec{v} \in \mathbb{R}^n \, \bvec{v} + (-\bvec{v}) = \bvec{0}$.
...
Scalar multiplication is associative, meaning $c(d\bvec{v}) = (cd)\bvec{v}$.

## Distributive Law

Scalar multiplication is distributive over vector addition, meaning $c(\bvec{v} + \bvec{w}) = c\bvec{v} + c\bvec{w}$.
...
export function DistributiveLaw() {
    ...
    return (
        <Mafs>
            ...
            />
            <LaTeX
                tex={String.raw`2\bvec{v} + 2\bvec{w}`}
...
This means that $1\bvec{v} = \bvec{v}$.
...
For the basis vectors, let $\bvec{P}$ be a point in space. For that point, the basis vectors are:

-   $\hat{\bvec{i}}_P$: The basis vector in the direction of the $x$-axis.
-   $\hat{\bvec{j}}_P$: The basis vector in the direction of the $y$-axis.
-   $\hat{\bvec{k}}_P$: The basis vector in the direction of the $z$-axis.

If another point $\bvec{Q}$ is chosen, the basis vectors remain the same; $\hat{\bvec{i}}_Q = \hat{\bvec{i}}_P$, $\hat{\bvec{j}}_Q = \hat{\bvec{j}}_P$, and $\hat{\bvec{k}}_Q = \hat{\bvec{k}}_P$.
Therefore, the references are dropped and the basis vectors are denoted as $\hat{\bvec{i}}$, $\hat{\bvec{j}}$, and $\hat{\bvec{k}}$.
...
Hence, references are kept, and for point $\bvec{P}$, the basis vectors are:

-   $\hat{\bvec{r}}_P$: The basis vector in the direction of the $r$-axis.
-   $\hat{\bvec{\theta}}_P$: The basis vector in the direction of the $\theta$-axis.
-   $\hat{\bvec{k}}_P$: The basis vector in the direction of the $z$-axis.
...
The cross product of two vectors $\bvec{v}$ and $\bvec{w}$ is denoted by $\bvec{v} \times \bvec{w}$.
