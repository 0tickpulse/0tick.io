...
$$
\bvec{E} = \frac{\bvec{F}}{q}
$$

Where:

-   $\bvec{E}$ is the electric field.
-   $\bvec{F}$ is the force on a test charge.
-   $q$ is the magnitude of the test charge.

$\bvec{E}$ hence has units of force per charge, or $\text{N} \text{C}^{-1}$.
...
$$
\begin{align*}
\left| \bvec{E} \right| &= \frac{\left| \bvec{F} \right|}{q} \\
