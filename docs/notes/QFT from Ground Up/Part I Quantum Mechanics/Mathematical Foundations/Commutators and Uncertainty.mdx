...
$$
\begin{equation}
\comm{\hat{A}, \hat{B}]^\dagger = \hat{B}\hat{A} - \hat{A}\hat{B} = -\qty[\hat{A}}{\hat{B}}
\end{equation}
$$
...
$$
\begin{equation}
\acomm{\hat{A}, \hat{B}}^\dagger = \qty(\hat{A}\hat{B} + \hat{B}\hat{A})^\dagger = \hat{B}^\dagger\hat{A}^\dagger + \hat{A}^\dagger\hat{B}^\dagger = \hat{B}\hat{A} + \hat{A}\hat{B} = \qty{\hat{A}}{\hat{B}}
\end{equation}
$$
...
The commutator $\comm{\Delta \hat{A}, \Delta \hat{B}]$ is simply equal to $\qty[\hat{A}}{\hat{B}}$.
