...
To take an example, consider taking the expectation value of $x$ at a certain time $t$, $\expval{x}(t)$:

- In the Schrödinger picture, we have:

    $$
    \begin{equation}
    \expval{x}(t) = \class{blue}{\bra{\psi} U^\dagger(t)} x \class{yellow}{U(t) \ket{\psi}} = \class{blue}{\bra{\psi(t)}} x \class{yellow}{\ket{\psi(t)}}
    \end{equation}
    $$
...
- In the Heisenberg picture, we have:
    ...
    $$
    \begin{equation}
    \expval{x}(t) = \bra{\psi} \class{green}{U^\dagger(t) x U(t)} \ket{\psi} = \bra{\psi} \class{green}{x(t)} \ket{\psi}
    \end{equation}
    $$
