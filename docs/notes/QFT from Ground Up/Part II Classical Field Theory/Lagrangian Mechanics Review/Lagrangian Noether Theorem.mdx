...
<Boxed>
**Noether's Theorem**: If a system is defined by a Lagrangian $L(q, \dot{q}, t)$, if, under a continuous infinitesimal transformation $q \to q + \epsilon \delta q$, the Lagrangian changes as $L \to L + \epsilon \dv{F}{t}$, then there exists a quantity $Q$:
...
$$
\begin{equation}
L = \frac{1}{2} mv^2 - V(\va{r})
\end{equation}
$$

where $V(\va{r})$ is the potential energy of the particle.
Notice that $V(\va{r})$ is a function of the position $\va{r}$, and not the velocity $\va{v}$.
Since $\va{F} = -\grad{V}$ and $\va{F} = \va{0}$, we have $\grad{V} = \va{0}$, which implies that $V$ is a constant.
Therefore, we can write $V(\va{r}) = V_0$, where $V_0$ is a constant:
