...
A straight line can be modeled as a spacetime separation vector $\va{S}$.
Then, its magnitude is given by:

$$
\begin{equation}
\begin{split}
\norm{\va{S}}^2 &= \va{S} \cdot \va{S} \\
&= \qty(\sum_\mu S^\mu \va{e}_\mu) \cdot \qty(\sum_\nu S^\nu \va{e}_\nu) \\
&= \sum_{\mu\nu} S^\mu S^\nu \va{e}_\mu \cdot \va{e}_\nu \\
&= \sum_{\mu\nu} S^\mu S^\nu \eta_{\mu\nu}
\end{split}
\end{equation}
$$
...
In the observer's frame, the observer is at rest, meaning that $\va{v} = \va{0}$.
