...
This will be a very lengthy and detailed set of notes.
Just to provide a feeling of the scope, here are just a few topics we will cover:

- We start not knowing what quantum mechanics is.
- In the quantum mechanics section, we will cover the Schrödinger picture, the Heisenberg picture, the interaction picture, and the path integral formulation.
- We will fully derive the nonrelativistic propagator for the simple harmonic oscillator, leading to the Mehler kernel, completely from first principles.
- We will thoroughly explore how quantum mechanics is used for quantum computation.
- We will derive Einstein's field equations from scratch using the Einstein-Hilbert action, and explore the concept of gauge invariance in general relativity.
- We will derive Schwarzschild's, Kerr's, Reissner-Nordström's, and the FLRW metric from the field equations.
    This includes the painstakingly detailed calculation of all the connection coefficients, Riemann tensor components, and so on.
- We will explore the implications of these metrics, such as black holes, gravitational waves, and cosmological theories.
- We will fully derive the Euler-Lagrange equations.
- We will introduce Clifford algebras and their wide-ranging applications in physics.
- We will use the vector calculus formalism, the tensor formalism, the Clifford algebra formalism, and the differential form formalism to describe electromagnetism.
    Electromagnetism is a U(1) gauge theory, and we will explore the implications of this gauge symmetry for charge conservation and the existence of gauge bosons.
- We will explore renormalization, effective field theories, and the concept of asymptotic freedom.
- We will explore the concept of spontaneous symmetry breaking and its implications for particle physics.
- We will explore various quantum field theories, such as quantum electrodynamics (QED), quantum chromodynamics (QCD), and the electroweak theory.
- We will explain each and every term in the Standard Model Lagrangian, including the Higgs mechanism and the generation of mass.
- We will explore the various applications of QFT, such as the Casimir effect, Hawking radiation, and the Unruh effect.
- We will explain why and how QFT and gravity are unreconcilable, and why we need a theory of quantum gravity.
