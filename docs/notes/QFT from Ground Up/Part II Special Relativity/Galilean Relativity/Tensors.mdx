...
A **tensor** is a mathematical object that is invariant under a change of basis.
...
We will first explore some examples of tensors, like covectors and linear maps.
...
A **covector** is a linear map that takes a vector and returns a scalar.
They have many names—dual vectors, one-forms, linear functionals, etc.—but they all refer to the same concept.
If you have read the quantum mechanics section, you will know that the bra vector $\bra{\psi}$ is a covector.
The Riesz representation theorem states that every covector can be represented as an inner product with a vector.
We will re-introduce the concept of covectors, this time in the context of tensors.

Let $\alpha$ be a covector. When applied to a vector $\vb{v}$, it returns a scalar, denoted by $\alpha(\vb{v})$.
They are represented by a row vector.
For a simple example, consider the covector $\alpha = \mqty[1 & 2]$.
...
From these examples, we can see that covectors are **linear** maps, meaning that they satisfy the following properties:
...
Hence, more formally, we define covectors symbolically as:
...
The set of all covectors on a vector space $V$ is called the **dual space** of $V$, denoted by $V^*$.

### Index Notation for Covectors

When we apply a covector to a vector, we take each component of the covector and multiply it by the corresponding component of the vector.
For a covector $\alpha = \mqty[3 & 9 & 5]$, we say that $\alpha_1 = 3$, $\alpha_2 = 9$, and $\alpha_3 = 5$.
...
Suppose we have covectors acting on two-dimensional vectors.
We can imagine a covector as a surface, where the height of the surface at a point $(x, y)$ is given by the value of the covector at that point.
For example, consider the covector $\alpha = \mqty[1 & 2]$. This is visualized as a plane with a slope of $2$ in the $x$-direction and $1$ in the $y$-direction:
...
We instead prefer to use a series of lines to represent covectors, akin to a contour plot.
Each line represents a constant value of the covector.
Then, the value of $\alpha$ on a vector $\vb{v}$ is given by how many lines $\vb{v}$ intersects.

export function CovectorContourVisualizer({ alpha = [1, 2], arrowsInsteadOfNumbers = false }) {
...
<CovectorContourVisualizer />

Alternatively, instead of writing the number on each line, we simply draw an arrow pointing in the direction of increasing values of the covector.

### Covector Arithmetic

Covectors can be added and multiplied by scalars in the same way as vectors.
This is because the dual space $V^*$ is itself a vector space.

When a covector is scaled, say $\alpha \to 2\alpha$, the contour lines look more densely packed.
When a covector is added to another, say $\alpha + \beta$, the contour lines are the sum of the two covectors.
...
### Covector Basis and Components

Just like vectors, covectors have a basis.
...
where $\delta^i_j$ is the Kronecker delta. Thus a covector can be written as:
...
Notice that unlike vector bases, covector bases have a superscript index—covector bases are contravariant.
To see this more concretely, let $T$ be the transformation matrix from an old basis to a new basis.
Suppose we apply a covector $\alpha$ to a vector $\vb{v}$. In the old basis, we have:
...
We already know that vector components are contravariant and transform with the inverse matrix. As such, $T^{-1} \mqty[\class{pink}{v^1} \\ \class{pink}{v^2}] = \mqty[\class{blue}{\tilde{v}^1} \\ \class{blue}{\tilde{v}^2}]$:

$$
\begin{equation}
\alpha(\vb{v}) = \mqty[\class{pink}{\alpha_1} & \class{pink}{\alpha_2}] T \mqty[\class{blue}{\tilde{v}^1} \\ \class{blue}{\tilde{v}^2}]
\end{equation}
$$
...
$$
\begin{equation}
\alpha(\vb{v}) = \mqty[\class{blue}{\tilde{\alpha}_1} & \class{blue}{\tilde{\alpha}_2}] \mqty[\class{blue}{\tilde{v}^1} \\ \class{blue}{\tilde{v}^2}]
\end{equation}
$$

Comparing the two expressions, we find that $\mqty[\class{blue}{\tilde{\alpha}_1} & \class{blue}{\tilde{\alpha}_2}] = \mqty[\class{pink}{\alpha_1} & \class{pink}{\alpha_2}] T$.
Hence, covector components are **covariant** and transform with the transformation matrix.
And by the fact that covectors are invariant, we have that the covector basis transforms with the inverse matrix.
...
A more general concept than covectors is that of a **linear map**.
...
$$
\begin{equation}
L \overset{.}{=} \mqty[\class{pink}{L^1_1} & \class{pink}{L^1_2} \\ \class{pink}{L^2_1} & \class{pink}{L^2_2}]_{\class{red}{\vb{e}_i}} = \mqty[? & ? \\ ? & ?]_{\class{blue}{\tilde{\vb{e}}_i}}
\end{equation}
$$
...
$$
\begin{equation}
\class{blue}{\tilde{L}}\qty(\mqty[\class{blue}{\tilde{v}^1} \\ \class{blue}{\tilde{v}^2}]) = T^{-1} \class{pink}{L} T \qty(\mqty[\class{blue}{\tilde{v}^1} \\ \class{blue}{\tilde{v}^2}])
\end{equation}
$$
...
$$
\begin{equation}
L(\class{darkblue}{\tilde{\vb{e}}_i}) = \class{blue}{\tilde{L}^q_i} \class{darkblue}{\tilde{\vb{e}}_q}
\end{equation}
$$
...
\begin{equation}
\begin{split}
\class{blue}{\tilde{L}^q_i} \class{darkblue}{\tilde{\vb{e}}_q} = L(\class{darkblue}{\tilde{\vb{e}}_i}) &= L\qty(T^j_i \class{red}{\vb{e}_j}) \\
...
$$
\begin{equation}
\class{blue}{\tilde{L}^l_i} \class{darkblue}{\tilde{\vb{e}}_l} = T^j_i \class{pink}{L^k_j} \qty(T^{-1})^l_k \class{darkblue}{\tilde{\vb{e}}_l}
\end{equation}
$$
...
$$
\begin{equation}
\class{blue}{\tilde{L}^l_i} = \qty(T^{-1})^l_k \class{pink}{L^k_j} T^j_i
\end{equation}
$$
...
$$
\class{blue}{\tilde{L}^l_i} = \qty(T^{-1})^l_{\class{pink}{k}} L^{\class{pink}{k}}_{\class{blue}{j}} T^{\class{blue}{j}}_i
$$

This is how we know that the entire matrix $\class{blue}{\tilde{L}}$ is given by $T^{-1} \class{pink}{L} T$ and not any other order.
...
- **Form**: A function that takes vectors and outputs a scalar. For example, covectors are 1-forms.
...
This is known as the **spectral decomposition** of an operator.
The fundamental idea is that the outer product of a vector and covector gives a linear map:
...
Since the outer product involves scaling one vector by two different scalars (in the covector), each column is a scaled version of the vector.
...
Formally, each basis pure matrix is the **tensor product** of the basis vectors and covectors.
...
In fact, a tensor can be defined as a set of vectors and covectors that are combined using tensor products.
...
$$
\begin{equation}
L = \class{pink}{L^i_j} \class{red}{\vb{e}_i} \otimes \class{red}{\vb{e}^j} = \class{pink}{L^i_j} \qty[\qty(T^{-1})^k_i \class{blue}{\tilde{\vb{e}}_k}] \otimes \qty[\qty(T)^j_l \class{blue}{\tilde{\vb{\epsilon}}^l}] = \qty[\qty(T^{-1})^k_i \class{pink}{L^i_j} T^j_l] \class{blue}{\tilde{\vb{e}}_k} \otimes \class{blue}{\tilde{\vb{\epsilon}}^l} = \class{blue}{\tilde{L}^k_l} \class{blue}{\tilde{\vb{e}}_k} \otimes \class{blue}{\tilde{\vb{\epsilon}}^l}
\end{equation}
$$

Thus it is clear that $L$ transforms as $\class{blue}{\tilde{L}} = T^{-1} \class{pink}{L} T$.
...
To take an example, a bilinear form is a tensor product of two covectors:
